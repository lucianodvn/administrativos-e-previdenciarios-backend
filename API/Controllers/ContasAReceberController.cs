...
namespace API.Controllers
{
    ...
    public class ContasAReceberController : ControllerBase
    {
        ...
        public ContasAReceberController(ContasAReceberService contasAReceberService)
        {
        ...
        public async Task<IActionResult> BuscarTodasContasAReceber()
        {
            ...
            var valorTotal = contasAReceberResponse.Sum(x => x.ValorTotal);
            var valorEntrada = contasAReceberResponse.Sum(x => x.ValorEntrada);
            var valorParcela = contasAReceberResponse.Sum(x => x.ValorParcela);
            var totalAReceber = valorTotal + valorEntrada + valorParcela;

            return Ok(new
            {
                Contas = contasAReceberResponse,
